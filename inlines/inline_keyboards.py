...
from configs.customs import (commands_queries, artist_commands_queries,
                             bot_settings_config, inline_textes,
                             search_methods, donation)

__back_keyboard = [[
    InlineKeyboardButton(inline_textes['back']['text'],
                         callback_data="/back_home")
]]

__commands_queries = list(commands_queries.values())
...
def create_keyboad_search(to_search):
    keyboad_search = [[
        ...
        for command in __commands_queries[line:line + 2]
...
__artist_commands_queries = list(artist_commands_queries.values())
...
def create_keyboard_artist(link):
    ...
    keyboad_artist = [[
        ...
        for command in __artist_commands_queries[line:line + 2]
    ...
    for line in range(0, __l_bot_settings, 2):
        ...
        for line2 in range(line, line + 2):
            ...
            c_keyboard = InlineKeyboardButton(
                msg, callback_data=f"/edit_setting_{cmd}")
...
__qualities = list(qualities.keys())
...
def create_keyboard_qualities():
    keyboad_qualities = [[
        InlineKeyboardButton(quality,
                             callback_data=f"/edit_setting_quality_{quality}")
        for quality in __qualities[line:line + 2]
...
def create_keyboard_search_method():
    keyboad_search_methods = [[
        ...
        for search_method in search_methods[line:line + 2]
...
def create_shazamed_keyboard(track_link, album_link, artist_link):
    keyboard_shazamed = [
        [
            InlineKeyboardButton(inline_textes['download_track']['text'],
                                 callback_data=f"/down:{track_link}")
        ],
        [
            InlineKeyboardButton(inline_textes['download_album']['text'],
                                 callback_data=f"/down:{album_link}")
        ],
        [
            InlineKeyboardButton(inline_textes['download_artist']['text'],
...
def create_c_dws_user_keyboard(dws: dict):
    keyboad_dws = [
        InlineKeyboardMarkup([[
            InlineKeyboardButton("Press here to delete this download 🚫",
...
def create_donation_keyboard():
    keyboard_donation = [[
        InlineKeyboardButton("💸 PAYPAL DONATION HERE", url=donation)
    ]]
