...
from configs.bot_settings import (output_songs, supported_link, output_shazam,
                                  log_errors, log_downloads, log_uploads,
                                  logs_path)

from os import (walk, listdir, mkdir, remove, system, name as os_name)

from os.path import (getsize, join, islink, isdir, exists)


def check_config_file(config):
    if not "token" in config['deez_login']:
        print(
            "Something went wrong with the login token in the configuration file"
        )
        exit()

    if not "session_string" in config['pyrogram']:
        print("Something went wrong with pyrogram in the configuration file")
        exit()

    if not "bot_token" in config['telegram']:
        print(
            "Something went wrong with the telegram token in the configuration file"
        )
        exit()

    if (not "key" in config['acrcloud']) or (
            not "secret" in config['acrcloud']) or (not "host"
                                                    in config['acrcloud']):
...
def is_supported_link(link):
    ...
    if not any(c_link == netloc for c_link in supported_link):
        return False
...
def set_path(tag, song_quality, file_format, method):
    ...
    l_encoded = len(song_name.encode())
