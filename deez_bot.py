...
from configs.customs import (not_found_query_gif, shazam_audio_query,
                             ...
                             help_msg, help_photo, feedback_text,
                             what_can_I_do, donate_text, reasons_text)
...
from utils.utils import (is_supported_link, get_download_dir_size,
...
from inlines.inline_keyboards import (
    create_keyboad_search, create_keyboard_settings, create_keyboard_qualities,
    create_shazamed_keyboard, create_keyboard_search_method,
    create_banned_keyboard, create_c_dws_user_keyboard,
...
def audio_handler(update: Update, context):
    ...
    if audio_size > recorded_file_max_size:
        bot.send_message(
            chat_id=chat_id,
            text=
            f"PLEASE SEND A FILE AUDIO/RECORD LOWER THAN {recorded_file_max_size} MB =)"
        )
    ...
    if track_link:
        reply_markup = create_shazamed_keyboard(track_link, album_link,
                                                artist_link)
    else:
        reply_markup = None
...
settings_handler = CommandHandler("settings",
                                  settings_command,
                                  filters=~to_ban,
                                  run_async=True)
...
quality_handler = CommandHandler("quality",
                                 quality_command,
                                 filters=~to_ban,
                                 run_async=True)
...
managing_banned_handler = CommandHandler("managing_banned",
...
add_banned_handler = CommandHandler("add_banned",
                                    add_banned_command,
                                    filters=to_root,
                                    run_async=True)
...
shazam_handler = CommandHandler("shazam",
                                shazam_command,
                                filters=~to_ban,
                                run_async=True)
...
kill_dw_handler = CommandHandler("kill_dw",
                                 kill_dw_command,
                                 filters=~to_ban,
                                 run_async=True)
...
info_handler = CommandHandler("info",
                              info_command,
                              filters=~to_ban,
                              run_async=True)
...
reasons_handler = CommandHandler("reasons",
                                 reasons_command,
                                 filters=~to_ban,
                                 run_async=True)
...
feedback_handler = CommandHandler("feedback",
                                  feedback_command,
                                  filters=~to_ban,
                                  run_async=True)
...
donate_handler = CommandHandler("donate",
                                donate_command,
                                filters=~to_ban,
                                run_async=True)
...
send_global_msg_handler = MessageHandler(filter_owl_channel,
...
callback_queries = CallbackQueryHandler(handle_callback_queries,
                                        run_async=True)
